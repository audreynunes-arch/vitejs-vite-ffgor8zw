...
export default function Statistiques({ agenceId, onRetour }: Props) {
  ...
  const enCours = filtres.filter((d) => d.statut === 'en_cours').length;
  const clos = filtres.filter((d) => d.statut === 'clos').length;
  const annules = filtres.filter((d) => d.statut === 'annule').length;
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      ...
      >
        {card('En cours', enCours, '#185FA5')}
        {card('Clos', clos, '#0F6E56')}
